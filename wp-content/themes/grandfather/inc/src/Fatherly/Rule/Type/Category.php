 ...
 */
class Category extends Rule implements RuleContract
{
    public function initialize()
    {
        $this->fieldKey = 'rule_category';
        $this->fieldLabel = __('Category', 'fatherly');
    }
    ...
    public function validateRuleValue($valid, $value, $field, $input)
    {
        ...
        if (strpos($value, ",") !== false) {
            ...
            foreach ($slugs as $slug) {
                $cat = wpcom_vip_term_exists($slug, 'category');
                if ($cat === 0 || $cat === null) {
                    $valid = sprintf("The slug %s is not a valid category slug", $slug);
                    return $valid;
                }
            }
        } else {
            $cat = wpcom_vip_term_exists($value, 'category');
            if ($cat === 0 || $cat === null) {
                $valid = sprintf("The slug %s is not a valid category slug", $value);
                return $valid;
            }
        }
        return $valid;
    ...
    public function validateRulePassage($ruleSettings, $postID)
    {
        switch ($ruleSettings['rule_operator']) {
            case '==':
                if (has_category(explode(',', $ruleSettings['rule_value']))) {
                    return true;
                } else {
                    return false;
                }
