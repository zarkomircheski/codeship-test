...
if os.getenv('BITBUCKET_BRANCH') == "master":
    ENVIRONMENT = "production"
else:
    ENVIRONMENT = "staging"

VERSION_LABEL = strftime("%Y%m%d%H%M%S")
BUCKET_KEY = os.getenv('APPLICATION_NAME_' + ENVIRONMENT) + '/' + VERSION_LABEL + \
...
def upload_to_s3(artifact):
    ...
    try:
        client = boto3.client(
            's3',
            region_name=os.getenv('AWS_REGION_' + ENVIRONMENT)
        )
    ...
    try:
        client.put_object(
            ...
            Bucket=os.getenv('S3_BUCKET_' + ENVIRONMENT),
            Key=BUCKET_KEY
        )
    except ClientError as err:
...
def deploy_new_revision():
    ...
    try:
        response = client.create_deployment(
            applicationName=str(os.getenv('APPLICATION_NAME_' + ENVIRONMENT)),
            deploymentGroupName=str(os.getenv('DEPLOYMENT_GROUP_NAME_' + ENVIRONMENT)),
            revision={
                'revisionType': 'S3',
                's3Location': {
                    'bucket': os.getenv('S3_BUCKET_' + ENVIRONMENT),
            ...
            deploymentConfigName=str(os.getenv('DEPLOYMENT_CONFIG_' + ENVIRONMENT)),
